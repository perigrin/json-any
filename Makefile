...
#     ABSTRACT => q[Wrapper Class for the various JSON classes.]
#     AUTHOR => q[Chris Thompson, C<< <cthom at cpan.org> >>]
#     DIR => []
#     DISTNAME => q[JSON::Any]
#     NAME => q[JSON::Any]
#     NO_META => q[1]
...
#     test => { TESTS=>q[t/00-load.t t/01-JSON.t t/02-JSON-XS.t t/03-JSON-DWIW.t t/04-ENV.t t/pod-coverage.t t/pod.t] }
...
TEST_FILE = test.pl
TEST_FILES = t/00-load.t t/01-JSON.t t/02-JSON-XS.t t/03-JSON-DWIW.t t/04-ENV.t t/pod-coverage.t t/pod.t
TESTDB_SW = -d
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Wrapper Class for the various JSON classes.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Chris Thompson, C&lt;&lt; &lt;cthom at cpan.org&gt; &gt;&gt;</AUTHOR>' >> $(DISTNAME).ppd
